...
static LGATTServiceInfo g_uart_decl[] =
{
    ...
    {TYPE_CHARACTERISTIC, "08590F7E-DB05-467E-8757-72F6FAEB13D4", FALSE, VM_BT_GATT_CHAR_PROPERTY_NOTIFY | VM_BT_GATT_CHAR_PROPERTY_READ, VM_BT_GATT_PERMISSION_READ, 0}, // tx for periphral; rx for central
    {TYPE_CHARACTERISTIC, "08590F7F-DB05-467E-8757-72F6FAEB13D4", FALSE, VM_BT_GATT_CHAR_PROPERTY_WRITE, VM_BT_GATT_PERMISSION_WRITE, 0}, // rx for periphral; tx for central
    {TYPE_END, 0, 0, 0, 0, 0}
...
boolean LGATTSUart::onCharacteristicAdded(LGATTAttributeData &data)
{
    ...
    APP_LOG("LGATTSUart::onCharacteristicAdded f[%d] uuid[12] = [0x%x] len[%d]", data.failed, uuid->uuid[12], uuid->length);
    if (!data.failed)
    {
