...
boolean LBTServerClass::begin(const uint8_t* buf, const uint8_t* pinCode)
{
	...
	m_signal_write = vm_signal_create();
	m_signal_read = vm_signal_create();
...
void LBTServerClass::end(void)
{
	...
	vm_signal_clear(m_signal_write);
	vm_signal_destroy(m_signal_write);
	vm_signal_clear(m_signal_read);
	vm_signal_destroy(m_signal_read);
