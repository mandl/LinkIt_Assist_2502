...
static vm_bt_gatt_client_callback_t gatt_client_cb;
...
vm_bt_gatt_attribute_value_t		 gatt_char_value;
vm_bt_gatt_attribute_value_t		 gatt_desc_value;
...
{
    const vm_bt_uuid_with_length_t *intUUID = &uuid;
    int len = intUUID->length;
    size_t n = 0;
...
void gatt_client_register_client_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status, VMUINT8 *app_uuid)
{
    APP_LOG("[LGATT]gatt_client_register_client_callback - reg_ctx[0x%x] Start", reg_ctx); 

    LGATTClient *client = GATTCGetClientByUUID((vm_bt_uuid_struct*)app_uuid);
    if (!client)
    {
...
void gatt_client_vmt_scan_result_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, vm_bt_gatt_address_t *bd_addr, VMINT32 rssi, VMUINT8 eir_len, VMUINT8 *eir)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(client->_cntx.reg_ctx == reg_ctx)
	{
	    ...
	    for (i = 0; i < idx; i++)
        {
            if (GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[0] == bd_addr->data[0]
                 && GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[1] == bd_addr->data[1]
                 && GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[2] == bd_addr->data[2]
                 && GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[3] == bd_addr->data[3]
                 && GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[4] == bd_addr->data[4]
                 && GATT_CLIENT_BD_ADDR_LIST[i].bd_addr.addr[5] == bd_addr->data[5])
            {
	    ...
	    {
	        memcpy(&(GATT_CLIENT_BD_ADDR_LIST[idx].bd_addr), bd_addr, sizeof(vm_bt_gatt_address_t));
...
void gatt_client_connection_callback(const vm_bt_gatt_connection_t *conn, VMBOOL connected, const vm_bt_gatt_address_t *bd_addr)
{
	APP_LOG("[LGATT] gatt_client_connection_callback - Start, connected = %d, context_handle[0x%x] connection_handle[0x%x]", 
	    connected, conn->context_handle, conn->connection_handle);
    ...
    client = GATTCGetClient(conn->context_handle);
    if (!client)
    {
        APP_LOG("[LGATT] FATAL 3");
    }
	...
	if(connected)
	{
        ...
        memcpy(&client->cb_data.connection.bd_addr, bd_addr, VM_BT_GATT_ADDRESS_SIZE);
        //memcpy(&client->cb_data.conn, conn, sizeof(vm_bt_gatt_connection_t));
        client->_return_value = true;
	}
	else
	{
    ...
    client->_cntx.conn_ctx = conn->connection_handle;
    client->cb_data.conn.conn_ctx = conn->connection_handle;
...
void gatt_client_listen_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status)
{/*
	...
	if ((gatt_client->_cntx.reg_ctx == reg_ctx) && (NULL != reg_ctx))
	{
		if(status == VM_OS_STATUS_SUCCESS)
		{
			...
			vm_bt_gatt_client_scan(reg_ctx, VM_TRUE);
		}
	}
*/}

void gatt_client_set_adv_data_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status)
{
}

void gatt_client_search_complete_callback(vm_bt_gatt_connection_t *reg_ctx, VMBOOL status)
{
    APP_LOG("[LGATT] gatt_client_search_complete_callback [0x%x] status[%d]", reg_ctx, status);
    vm_bt_gatt_connection_t *conn = (vm_bt_gatt_connection_t*)reg_ctx;
    ...
    APP_LOG("[LGATT] gatt_client_search_complete_callback reg_ctx[0x%x] conn_ctx[0x%x] END", conn->context_handle, conn->connection_handle);
}
...
*/
    if (uuid && (client->cb_data.search_result.service_uuid_num < MAX_BLE_SERVICES))
    {
	    ...
	    memcpy(&client->cb_data.search_result.uuid[client->cb_data.search_result.service_uuid_num++], 
	        (void*)uuid, sizeof(vm_bt_gatt_service_info_t));
...
void gatt_client_get_characteristic_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_client_characteristic_t *ch, VM_BT_GATT_CHAR_PROPERTIES properties)
{
    ...
    if (status == VM_OS_STATUS_SUCCESS)
	{
        ...
        LGATTUUID svcUUID = *((vm_bt_uuid_with_length_t*)&(client->cb_data.characteristic.ch.svc_uuid.uuid));
        LGATTUUID startCharUUID = *((vm_bt_uuid_with_length_t*)&(client->cb_data.characteristic.start_char_uuid.uuid));
        LGATTUUID charUUID = *((vm_bt_uuid_with_length_t*)&(client->cb_data.characteristic.ch.ch_uuid.uuid));
...
void gatt_client_get_descriptor_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_client_descriptor_t *descr)
{
	...
	{
	    client->_return_value = true;
	    memcpy(&(client->cb_data.descriptor.start_descr_uuid), descr->descriptor_uuid, sizeof(lgatt_att_uuid));
	    ...
	    memcpy(&(client->cb_data.descriptor.descr.descr_uuid), descr->descriptor_uuid, sizeof(lgatt_att_uuid));
...
void gatt_client_get_included_service_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_service_info_t *svc_uuid, vm_bt_gatt_service_info_t *incl_svc_uuid)
{
}

void gatt_client_register_for_notification_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status, vm_bt_gatt_address_t *bd_addr, vm_bt_gatt_client_characteristic_t *ch)
{
    LGATTClient *client = GATTCGetClient(reg_ctx);
...
void gatt_client_notify_callback(vm_bt_gatt_connection_t *conn, vm_bt_gatt_address_t *bd_addr, vm_bt_gatt_client_characteristic_t *ch, vm_bt_gatt_attribute_value_t *value, VMBOOL is_notify)
{
    LGATTClient *client = GATTCGetClient(conn->context_handle);
...
/*
    ...
    */
    ...
    if (status == VM_OS_STATUS_SUCCESS)
    {
        ...
        LGATTUUID svcUUID = *((vm_bt_uuid_with_length_t*)&(client->cb_data.read_characteristic.ch.svc_uuid.uuid));
...
void gatt_client_write_characteristic_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_client_characteristic_t *ch)
{
    LGATTClient *client = GATTCGetClient(conn->context_handle);
...
void gatt_client_read_descriptor_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_client_descriptor_t *descr, vm_bt_gatt_attribute_value_t *value)
{
    LGATTClient *client = GATTCGetClient(conn->context_handle);
	APP_LOG("[LGATT] gatt_client_write_characteristic_callback - status = %d", status);
    if (!client)
...
void gatt_client_write_descriptor_callback(vm_bt_gatt_connection_t *conn, VMBOOL status, vm_bt_gatt_client_descriptor_t *descr)
{   
    LGATTClient *client = GATTCGetClient(conn->context_handle);
...
void gatt_client_execute_write_callback(vm_bt_gatt_connection_t *conn, VMBOOL status)
{
/*
...
void gatt_client_read_remote_rssi_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status, vm_bt_gatt_address_t *bd_addr, VMINT32 rssi)
{
	...
	memcpy(&(client->cb_data.read_remote_rssi.bd_addr), bd_addr, sizeof(vm_bt_gatt_address_t));
...
void gatt_client_get_device_type_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status, vm_bt_gatt_address_t *bd_addr, VM_BT_GATT_CLIENT_DEV_TYPE dev_type)
{
}

void gatt_client_callback_init(vm_bt_gatt_client_callback_t *gattc_cb)
{
    gattc_cb->register_client = gatt_client_register_client_callback;
    gattc_cb->scan_result = gatt_client_vmt_scan_result_callback ;
    gattc_cb->connection = gatt_client_connection_callback;
    gattc_cb->listen = gatt_client_listen_callback;
    gattc_cb->set_advertisement_data = gatt_client_set_adv_data_callback;
    gattc_cb->search_complete = gatt_client_search_complete_callback;
    gattc_cb->search_result = gatt_client_search_result_callback;
    gattc_cb->get_characteristic = gatt_client_get_characteristic_callback;
    gattc_cb->get_descriptor = gatt_client_get_descriptor_callback;
    gattc_cb->get_included_service = gatt_client_get_included_service_callback;
    gattc_cb->register_for_notification = gatt_client_register_for_notification_callback;
    gattc_cb->notify = gatt_client_notify_callback;
    gattc_cb->read_characteristic = gatt_client_read_characteristic_callback;
    gattc_cb->write_characteristic = gatt_client_write_characteristic_callback;
    gattc_cb->read_descriptor = gatt_client_read_descriptor_callback;
    gattc_cb->write_descriptor = gatt_client_write_descriptor_callback;
    gattc_cb->execute_write = gatt_client_execute_write_callback;
    gattc_cb->read_remote_rssi = gatt_client_read_remote_rssi_callback;
    gattc_cb->get_device_type = gatt_client_get_device_type_callback;
	return;
}
...
void gatt_client_init(void *user_data)
{
	...
	if(client->_cntx.state == (lgatt_client_status)GATT_CLIENT_STATUS_DISABLED)
	{
		...
		vm_bt_gatt_client_register((const VMUINT8 *)&client->_cntx.uuid, (vm_bt_gatt_client_callback_t*)&gatt_client_cb);
	}
...
boolean gatt_client_deinit(void *userData)
{
    ...
    if (0 > vm_bt_gatt_client_deregister(client->_cntx.reg_ctx))
    {
        return false;
    }
    // TODO:
...
void gatt_client_bt_init_cb(VM_BT_CM_EVENT evt,void * param,void * user_data)
{
    APP_LOG("[LGATT]gatt_client_bt_init_cb - Start"); 
    if (VM_BT_CM_EVENT_ACTIVATE == evt )
    {
...
static void gatt_client_timeout_cb(VM_TIMER_ID_PRECISE tid, void* user_data)
{
	...
	vm_bt_gatt_client_scan(client->_cntx.reg_ctx,VM_FALSE);
...
boolean gattBegin(void *userData)
{
    ...
    // open BT
	vm_bt_cm_init(gatt_client_bt_init_cb,VM_BT_CM_EVENT_ACTIVATE | 0x00080000,userData);
...
boolean gattScan(void *userData)
{
    ...
    client->_cntx._tid = vm_timer_create_precise(client->_cntx._timeout*1000, gatt_client_timeout_cb, NULL);
    if (0 > client->_cntx._tid)
    {
        client->_return_value = false;
        return true;
	...
	client->scan_ret = vm_bt_gatt_client_scan(client->_cntx.reg_ctx,VM_TRUE);
	...
	{
	    vm_timer_delete_precise(client->_cntx._tid);
	    client->_cntx._tid = -1;
	}
...
boolean gattConnect(void *userData)
{
	...
	vm_bt_gatt_client_connect(client->_cntx.reg_ctx, (vm_bt_gatt_address_t*)&(client->_cntx._address), VM_TRUE);
...
boolean gattDisConnect(void *userData)
{
	...
	vm_bt_gatt_client_disconnect((vm_bt_gatt_connection_t*)&(client->cb_data.conn), (vm_bt_gatt_address_t*)&(client->_cntx._address));
...
boolean gattEnableNotification(void *userData)
{
    ...
    vm_bt_gatt_service_info_t svc_uuid = {0};
    vm_bt_gatt_attribute_uuid_t ch_uuid = {0};

    svc_uuid.is_primary = client->_cntx._isPrimary;
    svc_uuid.uuid.inst = 0;
    memcpy(&(svc_uuid.uuid.uuid), &(client->_cntx._currServiceUUID), sizeof(vm_bt_uuid_with_length_t));

    ch_uuid.inst = 0;
    memcpy(&(ch_uuid.uuid), &(client->_cntx._currCharUUID), sizeof(vm_bt_uuid_with_length_t));
    ...
    {
        vm_bt_gatt_client_deregister_for_notification(client->_cntx.reg_ctx, 
                        (vm_bt_gatt_address_t*)&(client->_cntx._address), 
                        &ch);
    }
...
/*
    ...
    vm_bt_gatt_service_info_t svc_uuid;
    svc_uuid.is_primary = client->_cntx._isPrimary;
    svc_uuid.uuid.inst = 0;
    memcpy(&(svc_uuid.uuid.uuid), &(client->_cntx._currServiceUUID), sizeof(vm_bt_uuid_with_length_t));
    ...
    vm_bt_gatt_client_get_characteristic(
        ...
        (vm_bt_gatt_service_info_t*)&svc_uuid, 
        (vm_bt_gatt_attribute_uuid_t*)(client->cb_data.characteristic.start_char_uuid.uuid.len ? &(client->cb_data.characteristic.start_char_uuid) : NULL));
...
boolean gattGetDescriptor(void *userData)
{
    ...
    vm_bt_gatt_client_characteristic_t ch;
    vm_bt_gatt_service_info_t svc_uuid;
    vm_bt_gatt_attribute_uuid_t ch_uuid;
    ...
    vm_bt_gatt_client_get_descriptor(
        (vm_bt_gatt_connection_t*)&(client->cb_data.conn), 
        (vm_bt_gatt_client_characteristic_t*)&ch, 
        (vm_bt_gatt_attribute_uuid_t*)(client->cb_data.descriptor.start_descr_uuid.uuid.len ? &(client->cb_data.descriptor.start_descr_uuid) : NULL));
...
boolean gattReadCharacteristic(void *userData)
{
    ...
    ret = vm_bt_gatt_client_read_characteristic((vm_bt_gatt_connection_t*)&(client->cb_data.conn), 
            (vm_bt_gatt_client_characteristic_t*)&ch,
...
boolean gattReadDescriptor(void *userData)
{
    ...
    vm_bt_gatt_service_info_t svc_uuid;
    vm_bt_gatt_attribute_uuid_t ch_uuid;
    vm_bt_gatt_attribute_uuid_t descr_uuid;
    ...
    descr_uuid.inst = 0;
    memcpy(&(descr_uuid.uuid), &(client->_cntx._currDescUUID), sizeof(vm_bt_uuid_with_length_t));

    descr.svc_uuid = &svc_uuid;
    descr.ch_uuid = &ch_uuid;
    descr.descriptor_uuid = &descr_uuid;
...
boolean gattReadRemoteRSSI(void *userData)
{
	...
	ret = vm_bt_gatt_client_read_remote_rssi(client->_cntx.reg_ctx, &bd_addr);
