...
public:
    ...
    LGATTUUID(const vm_bt_uuid_with_length_t &uuid);
    const vm_bt_uuid_with_length_t* operator&() const { return &_uuid; }
    LGATTUUID& operator=(const vm_bt_uuid_with_length_t &uuid);
    virtual size_t printTo(Print& p) const;
    /* DOM-NOT_FOR_SDK-END */
private:
    void str2uuid(uint8_t *uuid_dst, const char *uuid) const;

    vm_bt_uuid_with_length_t _uuid;
};
