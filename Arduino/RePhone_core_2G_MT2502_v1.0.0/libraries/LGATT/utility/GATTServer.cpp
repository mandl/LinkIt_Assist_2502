...
{
    VMINT16 trans_id;
    VM_BT_GATT_ATTRIBUTE_HANDLE char_handle; // ok
    vm_bt_gatt_attribute_value_t value; // ok
}attr_data_t;
...
struct gatts_srv_ctx_t
{
    ...
    // profile info
    vm_bt_gatt_service_info_t srv_uuid;
    VM_BT_GATT_ATTRIBUTE_HANDLE srv_handle;

    vm_bt_gatt_address_t addr; // client connection identify
...
vm_bt_gatt_server_callback_t GATTService::_gatts_service_cb = {0};
...
static void int_gatt_str_to_uuid(const VMUINT8* str, vm_bt_uuid_with_length_t* uuid)
{
#if 0
    uuid->len = 16;
    ...
    uuid_str = (VMUINT8*)vm_malloc(len + 1);
    if (uuid_str != NULL)
    {
        ...
        case UUID_STR_LEN_2:
            ...
            break;
        default:
            APP_LOG("int_gatt_str_to_uuid length error");
            break;
        }
...
boolean GATTService::rBegin(void *userdata)
{
    ...
    //vm_bt_uuid_with_length_t bt_uuid = {0};
    //int_gatt_str_to_uuid(s->_gatts_service.uuid, &bt_uuid);
    //memcpy(s->_gatts_srv_ctx.bt_uuid, &bt_uuid, sizeof(vm_bt_uuid_with_length_t));
...
boolean GATTService::rSend(void *userdata)
{
    ...
    if (s)
    {
        if (LGATTService::ACTION_SEND == s->_action)
        {
            ...
            conn.connection_handle = SRV_CTX(s)->conn_ctx;
            conn.context_handle= SRV_CTX(s)->reg_ctx;
            
            APP_LOG("vm_bt_gatt_server_send_indication hdl[%d] cfm[%d] v[0x%x] len[%d]", 
                s->_attr_handle, s->_confirm_failed, s->_value, s->_value->len);
            if (0 > vm_bt_gatt_server_send_indication(&conn, s->_attr_handle, s->_confirm_failed, (vm_bt_gatt_attribute_value_t*)s->_value))
            {
                s->_result = false;
            }
            s->_result = true;         
        ...
        else if (LGATTService::ACTION_SEND_ACK == s->_action)
        {
            ...
            APP_LOG("vm_bt_gatt_server_send_response hdl[%d] failed[%d] v[0x%x] len[%d] tid[%d]", 
                s->_attr_handle, s->_confirm_failed, s->_value, s->_value->len, s->_trans_id);
            if (0 > vm_bt_gatt_server_send_response(&conn, s->_trans_id, (VM_OS_STATUS)s->_confirm_failed, s->_attr_handle, (vm_bt_gatt_attribute_value_t*)s->_value))
            {
                s->_result = false;
            }
            s->_result = true;         
...
VMINT GATTService::addDeclaration(LGATTService *service, VM_BT_GATT_ATTRIBUTE_HANDLE srvc_handle, ard_decl_type_enum type)
{
    VMINT ret = TYPE_END;
    vm_bt_uuid_with_length_t bt_uuid = {0};
    int i, find = 0;
    ...
    if (item != NULL && find)
    {
        ...
        case TYPE_SERVICE:
            {
                vm_bt_gatt_service_info_t srv_uuid = {0};
                srv_uuid.is_primary = 1;
                int_gatt_str_to_uuid(item->uuid, &bt_uuid);
                memcpy(&srv_uuid.uuid.uuid, &bt_uuid, sizeof(vm_bt_uuid_with_length_t));
                APP_LOG("vm_bt_gatt_server_add_service -s");
                vm_bt_gatt_server_add_service(SRV_CTX(service)->reg_ctx, &srv_uuid, 6);
                ...
                APP_LOG("vm_bt_gatt_server_add_service -e");
                ret = TYPE_SERVICE;
            }
            break;
        case TYPE_CHARACTERISTIC:
            {
                vm_bt_gatt_attribute_uuid_t att_uuid = {0};
                int_gatt_str_to_uuid(item->uuid, &bt_uuid);
                memcpy(&att_uuid.uuid, &bt_uuid, sizeof(vm_bt_uuid_with_length_t));
                APP_LOG("vm_bt_gatt_server_add_characteristic -s");
                vm_bt_gatt_server_add_characteristic(SRV_CTX(service)->reg_ctx, srvc_handle, &(att_uuid.uuid), item->prop, item->permission);
                APP_LOG("uuid[0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x][%d]", 
                    bt_uuid.uuid[0], bt_uuid.uuid[1], bt_uuid.uuid[2], bt_uuid.uuid[3],
                    bt_uuid.uuid[4], bt_uuid.uuid[5], bt_uuid.uuid[6], bt_uuid.uuid[7],
                    bt_uuid.uuid[8], bt_uuid.uuid[9], bt_uuid.uuid[10], bt_uuid.uuid[11],
                ...
                APP_LOG("vm_bt_gatt_server_add_characteristic -e");
        ...
        case TYPE_DESCRIPTOR:
            {
                ...
                int_gatt_str_to_uuid(item->uuid, &bt_uuid);
                memcpy(&att_uuid.uuid, &bt_uuid, sizeof(vm_bt_uuid_with_length_t));
                APP_LOG("vm_bt_gatt_server_add_descriptor -s");
                vm_bt_gatt_server_add_descriptor(SRV_CTX(service)->reg_ctx, srvc_handle, &att_uuid, item->permission);
                APP_LOG("uuid[0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x][%d]",
                    bt_uuid.uuid[0], bt_uuid.uuid[1], bt_uuid.uuid[2], bt_uuid.uuid[3],
                    bt_uuid.uuid[4], bt_uuid.uuid[5], bt_uuid.uuid[6], bt_uuid.uuid[7],
                    bt_uuid.uuid[8], bt_uuid.uuid[9], bt_uuid.uuid[10], bt_uuid.uuid[11],
                ...
                APP_LOG("vm_bt_gatt_server_add_descriptor -e");
                ret = TYPE_DESCRIPTOR;
            }
            break;
        case TYPE_END:
...
{
    if (!GATTService::_gatts_service_cb.register_server)
    {
        GATTService::_gatts_service_cb.register_server = register_server_callback;
        GATTService::_gatts_service_cb.connection = connection_callback;
        GATTService::_gatts_service_cb.listen = listen_callback;
        GATTService::_gatts_service_cb.service_added = service_added_callback;
        //service->_gatts_service_cb.included_service_added = included_service_added_callback;
        GATTService::_gatts_service_cb.characteristic_added = characteristic_added_callback;
        GATTService::_gatts_service_cb.descriptor_added = descriptor_added_callback;
        GATTService::_gatts_service_cb.service_started = service_started_callback;
        //service->_gatts_service_cb.service_stopped = service_stopped_callback;
        //service->_gatts_service_cb.service_deleted = service_deleted_callback;
        GATTService::_gatts_service_cb.request_read = request_read_callback;
        GATTService::_gatts_service_cb.request_write = request_write_callback;
        //service->_gatts_service_cb.request_exec_write = request_exec_write_callback;
        //service->_gatts_service_cb.response_confirmation = response_confirmation_callback;
        //s._gatts_service_cb.read_tx_power = read_tx_power_callback;
    }
...
VMINT GATTService::toRegist(LGATTService *service)
{
    ...
    return vm_bt_gatt_server_register((const VMUINT8 *)uuid.uu, &(GATTService::_gatts_service_cb));
}

//typedef void (*vm_bt_gatt_register_server_callback)(VM_BT_GATT_CONTEXT_HANDLE context, VMBOOL has_error, VMUINT8 app_uuid[16]);
void GATTService::register_server_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status, VMUINT8 *app_uuid)
{
    LGATTService *service = GATTServer::getCtxByUUID((vm_bt_uuid_struct*)app_uuid);
...
//typedef void (*vm_bt_gatt_service_added_callback)(VMBOOL has_error, VM_BT_GATT_CONTEXT_HANDLE context, vm_bt_gatt_service_info_t* service_id, VM_BT_GATT_SERVICE_HANDLE service);
void GATTService::service_added_callback(VMBOOL status, VM_BT_GATT_CONTEXT_HANDLE reg_ctx, vm_bt_gatt_service_info_t *srvc_id, VM_BT_GATT_SERVICE_HANDLE srvc_handle)
{
    ...
    if (service)
    {
        ...
        {            
			...
			uuid.length = 2;
            uuid.uuid[0] = srvc_id->uuid.uuid.uuid[1];
            uuid.uuid[1] = srvc_id->uuid.uuid.uuid[0];
            vm_bt_gatt_server_set_advertisement_data(SRV_CTX(service)->reg_ctx, 
                                0, 
                                1, 
                                1, 
                                0,
...
//typedef void (*vm_bt_gatt_characteristic_added_callback)(VMBOOL has_error, VM_BT_GATT_CONTEXT_HANDLE context, vm_bt_gatt_attribute_uuid_t* uuid, VM_BT_GATT_SERVICE_HANDLE service, VM_BT_GATT_CHARACTERISTIC_HANDLE characteristic);
void GATTService::characteristic_added_callback(VMBOOL status, VM_BT_GATT_CONTEXT_HANDLE reg_ctx, vm_bt_gatt_attribute_uuid_t *uuid, VM_BT_GATT_SERVICE_HANDLE srvc_handle, VM_BT_GATT_CHARACTERISTIC_HANDLE char_handle)
{
    ...
    if (service)
    {
        if (VM_OS_STATUS_SUCCESS == status)
        {
            ...
            //#define GATT_TRANSPORT_LE_BR_EDR         0x02    
                //APP_LOG("vm_bt_gatt_server_start_service", 0);
                //vm_bt_gatt_server_start_service(ctx->reg_ctx, srvc_handle); // TODO: what are those value mean?
                APP_LOG("add TYPE_DESCRIPTOR");
                //vm_bt_gatt_server_add_descriptor(ctx->reg_ctx,ctx->srv_handle, vm_bt_gatt_attribute_uuid_t * uuid,VM_GATT_PERMISSION permission)
                GATTService::addDeclaration(service, srvc_handle, TYPE_DESCRIPTOR);
                
            }
            else if (TYPE_END == type) // touch the end of profile
            {
                APP_LOG("vm_bt_gatt_server_start_service service[0x%x] reg_ctx[0x%x]", service, SRV_CTX(service)->reg_ctx);
                vm_bt_gatt_server_start_service(SRV_CTX(service)->reg_ctx, srvc_handle);
            }
        }
        else
        {
...
//typedef void (*vm_bt_gatt_descriptor_added_callback)(VMBOOL has_error, VM_BT_GATT_CONTEXT_HANDLE context, vm_bt_gatt_attribute_uuid_t* uuid, VM_BT_GATT_SERVICE_HANDLE service, VM_BT_GATT_DESCRIPTOR_HANDLE descriptor);
void GATTService::descriptor_added_callback(VMBOOL status, VM_BT_GATT_CONTEXT_HANDLE reg_ctx, vm_bt_gatt_attribute_uuid_t *uuid, VM_BT_GATT_SERVICE_HANDLE srvc_handle, VM_BT_GATT_DESCRIPTOR_HANDLE descr_handle)
{
    LGATTService *service = GATTServer::getCtxByHandle(reg_ctx, srvc_handle);
    ...
    if (service)
    {
        if (VM_OS_STATUS_SUCCESS == status)
        {
            ...
            {
                APP_LOG("vm_bt_gatt_server_start_service");
                vm_bt_gatt_server_start_service(SRV_CTX(service)->reg_ctx, srvc_handle);
            }
        }
    }
...
void GATTService::service_started_callback(VMBOOL status, VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VM_BT_GATT_SERVICE_HANDLE srvc_handle)
{
    LGATTService *service = GATTServer::getCtxByHandle(reg_ctx, srvc_handle);
    ...
    if (service)
    {
        ...
        {
			vm_bt_uuid_with_length_t uuid;
            APP_LOG("vm_bt_gatt_server_listen");
			
			uuid.length = 16;
			memcpy(&(uuid.uuid), (VMCHAR*)SRV_CTX(service)->bt_uuid.uu, uuid.length);
            ...
            vm_bt_gatt_server_set_advertisement_data(SRV_CTX(service)->reg_ctx, 
                FALSE, TRUE, FALSE,
                 ...
                 0, 0,
                 &uuid);
            vm_bt_gatt_server_listen(SRV_CTX(service)->reg_ctx, 1);
        }
        else
        {
            /*
...
void GATTService::listen_callback(VM_BT_GATT_CONTEXT_HANDLE reg_ctx, VMBOOL status)
{
...
void GATTService::connection_callback(const vm_bt_gatt_connection_t *conn, VMBOOL connected, const vm_bt_gatt_address_t *bd_addr)
{
    LGATTService *service = GATTServer::getCtxByCTX(conn->context_handle);
    APP_LOG("connection_callback connected [%d] srv[0x%x] [0x%x, 0x%x] -s", connected, service, conn->context_handle, conn->connection_handle);
    if (service)
    {
        gatts_q_node *node = new gatts_q_node(GATTS_EVENT_ON_CONNETION, service);
        ((LGATTConnection *)(node->data))->connected = connected;
        memcpy(((LGATTConnection *)node->data)->addr.addr, bd_addr->data, LGATT_BD_ADDR_SIZE);
        GATTServer::rEnQ(*node);
    }
}

void GATTService::request_read_callback(vm_bt_gatt_connection_t *conn, VMUINT16 trans_id, vm_bt_gatt_address_t *bd_addr,
                                      VM_BT_GATT_ATTRIBUTE_HANDLE attr_handle, VMUINT16 offset, VMBOOL is_long)
{
    ...
    if (service)
    {
        ...
        memcpy(read->bd.addr, bd_addr->data, LGATT_BD_ADDR_SIZE);
        GATTServer::rEnQ(*node);
        APP_LOG("request_read_callback trans_id[%d] OK", trans_id);
    }
    APP_LOG("request_read_callback -e");
}
void GATTService::request_write_callback(vm_bt_gatt_connection_t *conn, VMUINT16 trans_id, vm_bt_gatt_address_t *bd_addr,
                                       ...
                                       VMBOOL need_rsp, VMBOOL is_prep)
{
    ...
    if (service)
    {
        ...
        memcpy(written->bd.addr, bd_addr->data, LGATT_BD_ADDR_SIZE);
        written->value.len = value->length;
        memcpy(written->value.value, value->data, LGATT_ATT_MAX_VALUE_LEN);
        GATTServer::rEnQ(*node);
...
void GATTServer::btcm_cb(VM_BT_CM_EVENT evt, void* param, void* user_data)
{
...
boolean GATTServer::init(LGATTServerClass *server)
{
    ...
    {
        GATTServer::_hdl_btcm = vm_bt_cm_init(GATTServer::btcm_cb, 
...
boolean GATTServer::deinit(LGATTServerClass *server)
{
    if (0 < GATTServer::_hdl_btcm)
    {
        vm_bt_cm_exit(GATTServer::_hdl_btcm);
        GATTServer::_hdl_btcm = 0;
    }
...
LGATTService *GATTServer::getCtxByHandle(void *reg_ctx, VM_BT_GATT_ATTRIBUTE_HANDLE srvc_handle)
{
    for (int i = 0; (i < MAX_SERVICES && LGATTServer._gatts_service[i]); i++)
    {
        if (reg_ctx == SRV_CTX(LGATTServer._gatts_service[i])->reg_ctx && 
...
boolean GATTServer::rBegin(LGATTServerClass *server)
{
    ...
    VMINT status = vm_bt_cm_get_power_status();
    APP_LOG("GATTServer::rBegin vm_bt_cm_get_power_status [%d]", status);
    if (VM_BT_CM_POWER_ON == status)
    {
        server->_result = true;
        APP_LOG("GATTServer::rBegin VM_BT_CM_POWER_ON -E1");
        return true;
    }
    else if (VM_BT_CM_POWER_OFF == status)
    {

        vm_bt_cm_switch_on();
        APP_LOG("GATTServer::rBegin vm_bt_cm_switch_on -E2");
    }
...
boolean GATTServer::rEnd(LGATTServerClass *server)
{
    ...
    else if (VM_BT_CM_POWER_ON == vm_bt_cm_get_power_status())
    {
        vm_bt_cm_switch_off();
    }
...
//{
//    
//    static vm_bt_gatt_service_info_t srv_uuid;
...
//            
//
//            //srv_uuid = (vm_bt_gatt_service_info_t*)vm_malloc(sizeof(vm_bt_gatt_service_info_t));
//            
//            APP_LOG("srv_uuid [0x%x] 11 ", 0, &srv_uuid);
//            /*
//			srv_uuid.uuid.uuid.len = 16;
//            srv_uuid.is_primary = 1;
//			memcpy(srv_uuid.uuid.uuid.uuid, &g_svc_uid, sizeof(vm_bt_uuid_with_length_t));
//			vm_bt_gatt_server_add_service(reg_ctx, &srv_uuid, 1);
//			*/
...
//void connection_callback(vm_bt_gatt_connection_t *conn, VMBOOL connected, vm_bt_gatt_address_t *bd_addr)
//{
//    gatts_srv_ctx_t *ctx;
//    VM_ASSERT(conn);
//    APP_LOG("connection_callback connected [%d] [0x%x, 0x%x] -s", connected, connected, conn->context_handle, conn->connection_handle);
//    ctx = int_gatt_du_get_ctx_by_reg(conn->context_handle);
//    if (ctx)
//    {
...
//            APP_LOG("vm_bt_gatt_server_listen stop", 0);
//            memcpy(ctx->addr.addr, bd_addr->data, LGATT_BD_ADDR_SIZE);
//            ctx->status = 1;
//        }
//        else 
//        {
...
//            {
//                APP_LOG("vm_bt_gatt_server_listen again", 0);
//                vm_bt_gatt_server_listen(conn->context_handle, TRUE);
//                ctx->status = 0;
//            }
//        }
...
//        hdl.conn_ctx = conn->connection_handle;
//        hdl.reg_ctx = conn->context_handle;
//        if (bd_addr)
//        {
...
//            {
//                APP_LOG("vm_bt_gatt_server_start_service", 0);
//                vm_bt_gatt_server_start_service(ctx->reg_ctx, srvc_handle);
//            }
//        }
//        else
//        {
...
//        if (VM_OS_STATUS_SUCCESS == status)
//        {
//            APP_LOG("vm_bt_gatt_server_listen", 0);
//            vm_bt_gatt_server_listen(ctx->reg_ctx, 1);
//        }
//        else
//        {
//        
...
//        return NULL;
//
//	client = vm_malloc(sizeof(vm_node_t));
//	if (NULL == client)
//	    return NULL;
//
