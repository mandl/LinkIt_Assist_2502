...
extern vm_mutex_t server_mutex;
...
static void bt_server_btcm_cb(VM_BT_CM_EVENT evt, void * param, void * user_data)
{
    ...
    switch (evt)
    {
        case VM_BT_CM_EVENT_ACTIVATE:
        {
            ...
            else if (g_serverContext.server_status == BT_SERVER_ENDING)
            {
                ...
                else
                {
                    ...
                    {
                        vm_bt_cm_exit(g_serverContext.btcm_hdl);
        ...
        {
            APP_LOG("[BTC] VM_BT_CM_EVENT_SET_VISIBILITY");
            *(g_serverContext.waiting_result_p) = true;
            g_serverContext.waiting_result_p = NULL;
            g_serverContext.ptr->post_signal();
            break;
        ...
        case VM_BT_CM_EVENT_PAIR_INDICATION:
        {
        	  ...
        	  vm_bt_cm_send_passkey(&event->device_address, (VMUINT8*)LBTServer._pincode_buffer, VM_TRUE);
            break;
        }
        default:
        {
...
static void bt_server_accept_timeout_cb(VM_TIMER_ID_PRECISE tid, void* user_data)
{
    ...
    if (tid == g_serverContext.accept_tid && g_serverContext.waiting_for_accept)
    {
        ...
        vm_timer_delete_precise(g_serverContext.accept_tid);
...
static int bt_server_spp_read(void* data)
{
    ...
    ret = vm_bt_spp_read(g_serverContext.conn_id, (void*)readdata, LBT_SERIAL_BUFFER_SIZE);
...
static void bt_server_spp_cb(VM_BT_SPP_EVENT evt, vm_bt_spp_event_cntx_t * param, void * user_data)
{
    ...
    {        
        case VM_BT_SPP_EVENT_CONNECT:
        {
            if (cntx->result)
            {
                ...
                g_serverContext.conn_id = cntx->connection_id;
            }
            else
            {
                //connnet fail
            ...
            if(g_serverContext.waiting_result_p != NULL)
            {
                g_serverContext.waiting_result_p = NULL;
                g_serverContext.ptr->post_signal();
                bt_server_accept_timeout_cb(g_serverContext.accept_tid, NULL);
            }
            else
            {
                // reconnection
        ...
        case VM_BT_SPP_EVENT_DISCONNECT:
        {
            ...
            APP_LOG((char*)"[BTS]VM_BT_SPP_EVENT_DISCONNECT");
            break;
        }
        ...
        case VM_BT_SPP_EVENT_AUTHORIZE:
        {
            ...
            {
                vm_bt_cm_bt_address_t curr_addr = {0};
                vm_bt_cm_bt_address_t tmp = {0};
                vm_bt_spp_get_device_address(cntx->connection_id, &curr_addr);
                ...
                {
                    vm_bt_spp_reject(cntx->connection_id);
                    break;
                }
            }
            ...
            {
                VMINT ret = vm_bt_spp_accept(cntx->connection_id, g_serverContext.btspp_buf, g_serverContext.btspp_buf_size, g_serverContext.btspp_buf_size);
                g_serverContext.waiting_for_accept = false;
                vm_timer_delete_precise(g_serverContext.accept_tid);
                g_serverContext.accept_tid = -1;
            }
        ...
        case VM_BT_SPP_EVENT_BIND_FAIL:
        {
            break;
        }
...
static VMINT bt_server_init_spp(uint8_t* name)
{
    ...
    VMUINT evt_mask = VM_BT_SPP_EVENT_BIND_FAIL |
    ...
    g_serverContext.btspp_hdl = vm_bt_spp_open(evt_mask,bt_server_spp_cb,NULL);
    ...
    vm_bt_spp_set_security_level(g_serverContext.btspp_hdl,VM_BT_SPP_SECURITY_NONE);
    ...
    VMINT ret = vm_bt_spp_bind(g_serverContext.btspp_hdl, SPP_UUID);
    ...
    if(name != NULL)
    {
        ret = vm_bt_cm_set_host_name((VMUINT8*)name);
    }
    ...
    if (VM_BT_CM_VISIBILITY_ON == vm_bt_cm_get_visibility())
    {
        *(g_serverContext.waiting_result_p) = true;
        g_serverContext.waiting_result_p = NULL;
        g_serverContext.ptr->post_signal();
...
static boolean bt_server_end_spp()
{
    if(g_serverContext.btspp_hdl >= 0)
    {
        vm_bt_spp_close(g_serverContext.btspp_hdl);
...
boolean btServerBegin(void *userData)
{
    ...
    g_serverContext.btcm_hdl = vm_bt_cm_init(
        bt_server_btcm_cb,     
...
boolean btServerAccept(void *userData)
{
    ...
    g_serverContext.accept_tid = vm_timer_create_precise(pContext->time_out*1000, bt_server_accept_timeout_cb, NULL);
...
boolean btServerGetHostDeviceInfo(void *userData)
{
    ...
    if( VM_BT_CM_ERROR_SUCCESS == vm_bt_cm_get_host_device_info(&dev_info) )
    {
...
boolean btServerRead(void *userData)
{
    ...
    VMINT ret = vm_bt_spp_read(g_serverContext.conn_id, pContext->buf, pContext->len);
...
boolean btServerWrite(void *userData)
{
    ...
    VMINT ret = vm_bt_spp_write(g_serverContext.conn_id, pContext->buf, pContext->len);
    APP_LOG("vm_bt_spp_write ret[%d] [%s]", ret, pContext->buf);
